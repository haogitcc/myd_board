...
struct SYS_SETTINGS {
	...
	int target;
	//add 
	int wifiMode;	//0-Sta 1-Ap2g 2-Ap5g
	char apname[STRING_LEN_64];
	char appassword[STRING_LEN_64];
};
...
extern s_IP_SETTINGS ip_settings;
...
int sys_config_init(void)
{
	...
	sys_cfg_inset_int("system.wifiMode", &sysConfigs.wifiMode);
	sys_cfg_inset_string("system.apname", &sysConfigs.apname,STRING_LEN_64);
	sys_cfg_inset_string("system.appassword", &sysConfigs.appassword,STRING_LEN_64);
    return 0;
}

/** 
 * [sys_config_load_2 description]
 * @Author   YY
 * @DateTime 2019年5月9日T16:00:23+0800
 * @param    reset                    []
 * @param    ipInfo                   [description]
 */
void sys_config_load_2(int reset ,s_IP_SETTINGS* ipInfo)
{
	...
	sysConfigs.wifiMode = 0;
	strcpy(sysConfigs.apname, "FUWIT");
	strcpy(sysConfigs.appassword, "88888888");
    ...
    if(reset == 0) {		
    	...
    	//更改的数据写入配置文件
	    strcpy(sysConfigs.ip, ipInfo->ip);
	    strcpy(sysConfigs.netmask, ipInfo->netmask);
	    strcpy(sysConfigs.gateway, ipInfo->gateway);   
	    sysConfigs.serverport = ipInfo->port;
        sys_config_save();
    }
    else if(reset == 2){	//写入wifi信息
        if(0 == sys_cfg_read("system")) {
            //printf("Read system config file OK!\n");
        } else {
			sys_config_save();
        ...
        if(ipInfo->wifiMode == 0)
        {
        	sysConfigs.wifiMode = ipInfo->wifiMode; 
			strcpy(sysConfigs.wifiname, ipInfo->wifi);
			strcpy(sysConfigs.wifipassword, ipInfo->wifiwd); 
        }
        else
        {
			sysConfigs.wifiMode = ipInfo->wifiMode; 
...
int sysSettingGetString(const char* name, char* value, int valueLen, int searchFlag)
{
   ...
   if (!strcmp(name, "ip"))
	...
	else if(!strcmp(name, "apname"))
		strncpy(value, sysConfigs.apname, valueLen);
	else if(!strcmp(name, "appassword"))
		strncpy(value, sysConfigs.appassword, valueLen);
    else
